...
import 'package:in_action/app/dao/quiz/userLogin.dart';
...
class QuizDao {
  ...
  static Future<List<Quiz>> list(String text) async {
    final response = await dio
        .get('$url/list?keyword=$text&token=${AuthService.to.token.value}');
    List<Quiz> list =
  ...
  static Future<void> updateCount(num quizId, num remark) async {
    await dio.post(
        '$url/mark?quizId=$quizId&userId=${AuthService.to.userId.value}&remark=$remark&token=${AuthService.to.token.value}');
  }
